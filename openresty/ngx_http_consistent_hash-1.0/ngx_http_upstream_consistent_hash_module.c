...
static void inline ngx_swap(ngx_http_upstream_consistent_hash_node *a, 
        ngx_http_upstream_consistent_hash_node *b); 
        ...
        ngx_http_upstream_srv_conf_t *us)
{
    ...
    for (i = 0; i < us->servers->nelts; i++) {
        for (j = 0; j < server[i].naddrs; j++) {
            ...
            for (k = 0; k < (points_per_node * server[i].weight); k++) {
                ...
                // i is correct ? because a host may be have several ip address
    ...
    for (i = 0; i < MMC_CONSISTENT_BUCKETS; i++) {
        buckets->bucket[i] = ngx_http_upstream_consistent_hash_find(continuum, i * step);

        ngx_log_debug3(NGX_LOG_DEBUG_HTTP, cf->log, 0, 
                        "consistent hash bucket:  index: %d, point: %ui, server_num: %d", 
                        i, continuum->nodes[i].point, continuum->nodes[i].server_num);
    }
